...
const LegalOverlay: React.FC<LegalOverlayProps> = ({ type, onClose, profile, lang, t }) => {
  ...
  const legalContent =
    type === 'imprint'
      ? t.legal.imprint.content
      : type === 'privacy'
      ? t.legal.privacy.content
      : '';

  const legalContentResolved =
    typeof (legalContent as any).replaceAll === 'function'
      ? legalContent.replaceAll('{{DATE}}', legalDate)
      : legalContent.replace(/{{DATE}}/g, legalDate);
