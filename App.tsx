...
const App: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Language Switcher (more integrated / glassy) */}
      <div className="fixed top-4 left-4 z-[100] sm:top-6 sm:left-6">
        <div className="rounded-2xl bg-white/[0.035] border border-white/[0.08] backdrop-blur-md shadow-[0_18px_60px_-30px_rgba(0,0,0,0.9)]">
          <LanguageSwitcher currentLang={lang} onLanguageChange={setLang} />
        </div>
      </div>

      <main className="w-full max-w-[440px] relative pt-16 pb-20">
        {/* Halo behind the main card (premium separation) */}
        <div className="pointer-events-none absolute left-1/2 top-24 -translate-x-1/2 w-[520px] h-[780px] rounded-[4rem] blur-3xl opacity-60 bg-[radial-gradient(circle_at_50%_30%,rgba(255,255,255,0.08),transparent_55%),radial-gradient(circle_at_50%_85%,rgba(10,6,56,0.38),transparent_62%)]" />

        {/* Hauptkarte */}
        <div
          className={`relative bg-[#05031A]/96 rounded-[3.5rem] shadow-[0_55px_120px_-35px_rgba(0,0,0,0.95)] border border-white/[0.055] overflow-hidden flex flex-col items-center p-8 md:p-10 text-center transition-all duration-800 delay-100 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}
        >
          {/* Share Button (cleaner, less contrast) */}
          <button
            onClick={handleShare}
            className="absolute top-[8rem] right-7 p-2.5 rounded-full bg-white/[0.03] backdrop-blur-md border border-white/[0.10] hover:bg-white/[0.06] hover:border-white/[0.14] transition-all active:scale-90 z-10"
          >
            <svg
              className={`w-3.5 h-3.5 transition-colors ${copySuccess ? 'text-green-400' : 'text-white/45 group-hover:text-white/70 hover:text-white/70'}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="relative mb-8 group">
            <div className="w-40 h-40 rounded-full border border-white/10 p-1.5 bg-gradient-to-b from-white/10 to-transparent shadow-2xl transition-transform duration-700 group-hover:scale-[1.02]">
              <div className="w-full h-full rounded-full overflow-hidden bg-[#030016] border border-black/50">
                <img
          ...
          <div className="mb-10 w-full px-4">
            ...
            <div className="w-12 h-[1px] bg-white/25 mx-auto"></div>
          </div>

          {/* 4. Action Buttons */}
          <div className="w-full space-y-4 mb-10">
            {/* Save Contact Button (subtle premium gradient + refined shadow) */}
            <button
              onClick={handleDownloadVCard}
              disabled={isDownloading}
              className="w-full bg-gradient-to-b from-white to-white/90 text-[#0B1026] font-bold py-5 rounded-2xl flex items-center justify-center space-x-3 hover:to-white/80 transition-all active:scale-[0.98] shadow-[0_18px_40px_-18px_rgba(255,255,255,0.35)] relative overflow-hidden group"
            >
            ...
            {/* Quick Actions Grid (navy glass / less "blue button") */}
            <div className="grid grid-cols-3 gap-3">
              {/* Email */}
              <a
                href={`mailto:${USER_PROFILE.email}`}
                className="bg-white/[0.035] border border-white/[0.06] py-5 rounded-2xl flex flex-col items-center space-y-2 hover:bg-white/[0.06] hover:border-white/[0.10] transition-all group"
              >
                <svg className="w-5 h-5 text-white/65 group-hover:text-white/85 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2v10a2 2 0 002 2z" />
                </svg>

                <span className="text-[8px] font-bold uppercase tracking-[0.1em] text-white/55 group-hover:text-white/75">
                  {t.email}
                </span>
              </a>
              ...
              <a
                ...
                className="bg-white/[0.035] border border-white/[0.06] py-5 rounded-2xl flex flex-col items-center space-y-2 hover:bg-white/[0.06] hover:border-white/[0.10] transition-all group"
              >
                <svg className="w-5 h-5 text-white/65 group-hover:text-white/85 transition-colors" fill="currentColor" viewBox="0 0 24 24">
        ...
        {/* Legal Links (bigger tap target on mobile) */}
        <div
          key={lang}
          className={`mt-10 flex justify-center space-x-8 transition-all duration-800 delay-500 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
        >
          <button
            onClick={() => setActiveLegal('imprint')}
            className="py-2 px-1 text-[10px] uppercase tracking-[0.2em] text-slate-400 hover:text-slate-200 transition-colors"
          >
            {t.imprint}
          </button>
          <button
